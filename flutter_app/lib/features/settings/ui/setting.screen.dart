...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _loadProfile() async {
    try {
      final userResponse = await _client.get('/user');
      final planResponse = await _client.get('/plan/current');

      setState(() {
        _name = userResponse['name'] ?? '';
        _email = userResponse['email'] ?? '';
        _projectsCount = userResponse['projects_count'] ?? 0;
        _diagramsCount = userResponse['diagrams_count'] ?? 0;
        _userPlan = planResponse['plan'] ?? 'free';

        final expires = planResponse['expires_at'];
        if (expires != null) {
          final date = DateTime.tryParse(expires);
          if (date != null) {
            _planExpiresAt = '${date.day}/${date.month}/${date.year}';
          }
        }
        _isLoading = false;
      });
      ...
      body: SafeArea(
        child: CustomScrollView(
          slivers: [
            SliverToBoxAdapter(
              child: Padding(
                padding: EdgeInsets.fromLTRB(AppSizes.screenPadding, AppSizes.lg, AppSizes.screenPadding, AppSizes.md),
                child: Row(
                ...
                delegate: SliverChildListDelegate([
                    ...
                    child: Column(
                      children: [
                        ...
                        _buildTileItem(
                          ...
                          trailing: const Icon(Icons.visibility_off_outlined, size: 18, color: AppColors.textTertiary),
                          onTap: () {},
                        ),
                  ...
                  _buildCard(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text('Generation Engine', style: TextStyle(fontSize: AppSizes.fontSm, color: AppColors.textSecondary)),
                        SizedBox(height: AppSizes.sm),
                        _buildSegmentedControl(),
                        SizedBox(height: AppSizes.md),
                        _buildSwitchItem(label: 'Smart Enhancement', value: true, onChanged: (_) {}),
                        SizedBox(height: AppSizes.md),
                        Text('Analysis Depth', style: TextStyle(fontSize: AppSizes.fontSm, color: AppColors.textSecondary)),
                  ...
                  _buildCard(
                    ...
                    titleIconWidget: Icon(Icons.download_rounded, size: 16, color: AppColors.primary),
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Export Formats', style: TextStyle(fontSize: AppSizes.fontSm, color: AppColors.textSecondary)),
                        SizedBox(height: AppSizes.sm),
                        Wrap(
                          ...
                          children: ['PDF', 'Mermaid', 'JSON', 'PNG'].map((f) => Container(
                            ...
                            child: Text(f, style: TextStyle(fontSize: AppSizes.fontXs, color: AppColors.primary, fontWeight: FontWeight.w600)),
                          )).toList(),
                        ),
                        SizedBox(height: AppSizes.md),
                        Text('Integrations', style: TextStyle(fontSize: AppSizes.fontSm, color: AppColors.textSecondary)),
                        SizedBox(height: AppSizes.sm),
                        Row(
                          children: [
                  ...
                  _buildCard(
                    ...
                    child: Column(
                      children: [
                        _buildTileItem(icon: Icons.palette_rounded, label: 'Theme', trailing: Text('Dark', style: TextStyle(fontSize: AppSizes.fontSm, color: AppColors.textTertiary)), onTap: () {}),
                  ...
                  Center(child: Text('System & Support', style: TextStyle(fontSize: AppSizes.fontSm, color: AppColors.textTertiary))),
                  SizedBox(height: AppSizes.xl),
                ]),
              ),
            ),
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                Text(title, style: TextStyle(fontSize: AppSizes.fontMd, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
              ],
            ),
          ),
          Padding(padding: EdgeInsets.all(AppSizes.md), child: child),
        ],
      ),
    );
  }
  ...
  Widget _buildPremiumBanner() {
    final isPro = _userPlan == 'pro' || _userPlan == 'enterprise';
    final planName = _userPlan == 'enterprise' ? 'Enterprise' : _userPlan == 'pro' ? 'Pro' : 'Free';
    final planColors = _userPlan == 'enterprise'
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: planColors, begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(AppSizes.radiusMd),
      ),
      child: Row(
        children: [
          Icon(isPro ? Icons.workspace_premium_rounded : Icons.lock_outline_rounded, color: Colors.white, size: 32),
          SizedBox(width: AppSizes.sm),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('DiagramAI $planName', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: AppSizes.fontMd)),
                Text(
                  isPro
                      ? _planExpiresAt.isNotEmpty ? 'Expires: $_planExpiresAt' : 'Active'
                      : '10 diagrams / month',
                  style: TextStyle(color: Colors.white70, fontSize: AppSizes.fontSm),
                ),
              ],
            ),
          ),
          ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(
              ...
              foregroundColor: planColors[0],
              elevation: 0,
              padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppSizes.radiusSm)),
            ),
            child: Text(isPro ? 'Manage' : 'Upgrade', style: TextStyle(fontSize: AppSizes.fontSm, fontWeight: FontWeight.w600)),
          ),
        ],
      ),
    );
          ...
          children: [
            Row(
              children: [
                ...
                Text('Storage Management', style: TextStyle(fontSize: AppSizes.fontSm, color: AppColors.textPrimary, fontWeight: FontWeight.w500)),
                const Spacer(),
            ...
            Text('$_projectsCount Projects / $_diagramsCount Diagrams', style: TextStyle(fontSize: AppSizes.fontXs, color: AppColors.textTertiary)),
          ],
        ),
      ),
    );
  ...
  Widget _buildTileItem({required IconData icon, required String label, Widget? trailing, required VoidCallback onTap}) {
    return GestureDetector(
      ...
      child: Container(
        ...
        padding: EdgeInsets.symmetric(horizontal: AppSizes.sm, vertical: 10),
        decoration: BoxDecoration(color: AppColors.background, borderRadius: BorderRadius.circular(AppSizes.radiusSm), border: Border.all(color: AppColors.border)),
        child: Row(
          children: [
            Icon(icon, size: 16, color: AppColors.primary),
            SizedBox(width: AppSizes.sm),
            Expanded(child: Text(label, style: TextStyle(fontSize: AppSizes.fontSm, color: AppColors.textPrimary))),
  ...
  Widget _buildSwitchItem({required String label, required bool value, required ValueChanged<bool> onChanged, IconData? icon}) {
    return Padding(
      ...
      child: Row(
        children: [
          if (icon != null) ...[Icon(icon, size: 16, color: AppColors.primary), SizedBox(width: AppSizes.sm)],
          Expanded(child: Text(label, style: TextStyle(fontSize: AppSizes.fontSm, color: AppColors.textPrimary))),
  ...
  Widget _buildIntegrationIcon(IconData icon, Color color, String label) {
    return Column(
      children: [
        ...
        const SizedBox(height: 4),
        Text(label, style: TextStyle(fontSize: AppSizes.fontXs, color: AppColors.textTertiary)),
      ],
    );
  }
  ...
  Widget _buildDangerButton({required IconData icon, required String label, required VoidCallback onTap}) {
    return GestureDetector(
      ...
      child: Container(
        ...
        padding: EdgeInsets.symmetric(vertical: 12, horizontal: AppSizes.md),
        decoration: BoxDecoration(
        ...
        child: Row(
          ...
          children: [
            ...
            SizedBox(width: AppSizes.sm),
            Text(label, style: TextStyle(fontSize: AppSizes.fontSm, color: AppColors.error, fontWeight: FontWeight.w600)),
          ],
        ),
      ),
    );
  ...
  Widget _buildSignOutButton() {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Center(child: Text('Sign Out', style: TextStyle(fontSize: AppSizes.fontSm, color: Colors.white, fontWeight: FontWeight.w600))),
      ),
    );
  }
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Edit Profile', style: TextStyle(fontSize: AppSizes.fontXl, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
            SizedBox(height: AppSizes.lg),
              ...
              children: [
                Expanded(
                  child: OutlinedButton(
                    ...
                    style: OutlinedButton.styleFrom(foregroundColor: AppColors.textSecondary, side: const BorderSide(color: AppColors.border), padding: const EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppSizes.radiusMd))),
